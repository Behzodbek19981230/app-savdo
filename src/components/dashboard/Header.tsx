...
export function Header({ onMenuClick }: HeaderProps) {
    ...
    } = useExchangeRates(userFilialId ? { filial: userFilialId } : undefined);
    ...
    return (
        <>
            <header
                className={`flex-shrink-0 flex items-center justify-between bg-background border-b border-border px-4 lg:px-6 py-3 lg:py-4 lg:pb-3 ${shouldBlockInterface ? 'pointer-events-none opacity-50' : ''}`}
            >
                    ...
                    {filials.length > 0 && !shouldBlockInterface && (
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild >
                                <Button
                                    variant='outline'
                                    className='hidden py-4  md:flex items-center justify-start gap-2  rounded-xl shadow-sm min-w-[160px] max-w-[280px]'
                                    disabled={shouldBlockInterface}
                                >
                                    {selectedFilial?.logo ? (
                                        <img
                                            ...
                                            className='h-7 w-7 rounded-sm object-cover flex-shrink-0'
                                        />
                                    ) : (
                                        <Building2 className='h-4 w-4 flex-shrink-0' />
                                    )}
                                    <span className='truncate text-xs'>{selectedFilial?.name || 'Filial tanlang'}</span>
                                </Button>
                            </DropdownMenuTrigger>
                                            ...
                                            >
                                                <div className='w-full p-2.5'>
                                                    <div className='w-full flex items-start justify-between gap-2'>
                                                        <p className='text-xs font-semibold leading-5 line-clamp-1'>
                                                            {note.title || 'Sarlavha'}
                                                        </p>
                                                        <span
            ...
            <Dialog open={isExchangeDialogOpen} onOpenChange={handleExchangeDialogChange}>
                <DialogContent
                    ...
                    onPointerDownOutside={(e) => shouldBlockInterface && e.preventDefault()}
                    onEscapeKeyDown={(e) => shouldBlockInterface && e.preventDefault()}
                >
                    <DialogHeader>
            ...
            <Dialog open={isExchangeHistoryDialogOpen} onOpenChange={setIsExchangeHistoryDialogOpen}>
                <DialogContent className='sm:max-w-[900px] max-h-[80vh] overflow-y-auto'>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {selectedFilial
                                ? `${selectedFilial.name} filiali uchun dollar kursi tarixi`
                                : 'Dollar kursi tarixi'}
                        </DialogDescription>
                    </DialogHeader>
                    <div className='mt-4'>
                        {isExchangeHistoryLoading ? (
                            ...
                            <div className='text-center py-8 text-muted-foreground'>Tarix ma'lumotlari topilmadi</div>
                        ) : (
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {exchangeRatesList.map((rate, index) => (
                                        <TableRow key={rate.id}>
                                            ...
                                            <TableCell className='text-muted-foreground'>
                                                {rate.updated_by_detail?.full_name ||
                                                    rate.updated_by_detail?.username ||
                                                    '—'}
                                            </TableCell>
                                            <TableCell>
                                                <Button
            ...
            <Dialog open={isHistoryModalOpen} onOpenChange={setIsHistoryModalOpen}>
                <DialogContent className='sm:max-w-[700px] max-h-[80vh] overflow-y-auto'>
                    ...
                    <div className='mt-4'>
                        {selectedRateId && <ExchangeRateHistoryContent rateId={selectedRateId} />}
                    </div>
                    <DialogFooter>
            ...
            <Dialog open={isNoteDialogOpen} onOpenChange={setIsNoteDialogOpen}>
                <DialogContent className='sm:max-w-[560px]'>
                    ...
                    <div className='space-y-3'>
                        ...
                        <div className='rounded-lg border border-border bg-card p-3 text-xs leading-6 whitespace-pre-wrap'>
