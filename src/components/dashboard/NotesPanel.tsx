			...
			const constructedNote: NoteItem | null = note?.id
				...
				: payload.note_id
					? {
							...
							date: (payload.deadline as string) || undefined,
						}
					: null;
			...
			{!embedded && (
				<div className='flex items-center justify-between gap-2 border-b border-border px-4 py-2.5'>
					<div className='flex items-center gap-2'>
						<h3 className='text-xs font-semibold text-foreground'>Eslatmalar</h3>
						{unreadCount > 0 && (
					...
					<div className='space-y-4 py-2'>
						...
						{viewingNote?.text && (
							<div className='rounded-lg border border-border bg-muted/30 p-4'>
								<p className='text-xs whitespace-pre-wrap break-words'>{viewingNote.text}</p>
							</div>
						)}
						{!viewingNote?.text && <p className='text-xs text-muted-foreground italic'>Izoh mavjud emas</p>}
					</div>
					<DialogFooter>
