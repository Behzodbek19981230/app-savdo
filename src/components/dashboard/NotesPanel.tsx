...
export function NotesPanel({ embedded = false }: NotesPanelProps) {
	...
	// Prevent selecting past dates when creating a new note
	...
	useEffect(() => {
		const onMsg = (ev: Event) => {
			const payload = (ev as CustomEvent).detail as WsNotePayload;
			if (!payload) return;
			const action = payload.action || payload.type;
			const note = payload.note || (payload.id ? (payload as NoteItem) : null);
			// If server sends note_id style events, construct a partial NoteItem
			const constructedNote: NoteItem | null = note?.id
				? note
				: payload.note_id
					? {
							id: payload.note_id,
							title: (payload.note_title as any) || (payload.title as any) || undefined,
							text: (payload.message as any) || undefined,
							status: (payload.status as any) || undefined,
							date: (payload.deadline as any) || undefined,
						}
					: null;
			...
			else if (constructedNote?.id) setNotes((prev) => upsertNote(prev, constructedNote));

			// Show a toast for explicit note events
			if ((payload.type === 'note_event' || (payload as any).event) && toast) {
				toast({
					title: (payload.title as any) || (payload.note_title as any) || 'Eslatma',
					description: (payload.message as any) || (payload.message as any) || undefined,
				});
			}
		};
		...
		const onStatus = (ev: Event) => {
			...
			console.debug('[NotesPanel] notesws status', detail);
		};

		window.addEventListener('notes:message', onMsg as EventListener);
		window.addEventListener('notes:status', onStatus as EventListener);
		return () => {
			window.removeEventListener('notes:message', onMsg as EventListener);
			window.removeEventListener('notes:status', onStatus as EventListener);
		};
	}, []);
					...
					<div className='divide-y divide-border'>
						{sortedNotes.map((note) => {
							...
							return (
								<div
									key={note.id}
									className={`group flex items-center gap-2.5 px-4 py-2 transition-colors hover:bg-muted/50 cursor-pointer ${
			...
			<Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
				<DialogContent className='sm:max-w-[500px]'>
					...
					<div className='space-y-4 py-2'>
						...
						{!viewingNote?.text && <p className='text-sm text-muted-foreground italic'>Izoh mavjud emas</p>}
					</div>
					<DialogFooter>
					...
					<div className='grid gap-3 py-1'>
						...
						<div className='flex gap-2'>
							<DatePicker
								...
								placeholder='Sana'
								minDate={editingNoteId ? undefined : startOfToday}
							/>
							<Input
								type='time'
								value={reminderTime}
