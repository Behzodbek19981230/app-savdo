...
const statusConfig = {
	...
	cancelled: {
		label: 'Bekor qilindi',
		className: 'bg-destructive/[0.10] border-destructive/[0.22] text-destructive',
	},
};
	...
	return (
		<div className='rounded-xl lg:rounded-2xl border border-border bg-card p-4 lg:p-5 shadow-sm'>
			<div className='flex items-center justify-between gap-3 mb-4'>
				...
				<a href='#' className='text-xs lg:text-xs font-medium text-primary hover:underline whitespace-nowrap'>
					Barchasini ko'rish
				</a>
			</div>
				...
				{orders.map((order) => (
					...
					>
						<div className='flex items-center gap-2.5 lg:gap-3 min-w-0 flex-1'>
							<div className='flex h-7 w-9 items-center justify-center rounded-full bg-primary/[0.18] border border-primary/[0.22] flex-shrink-0'>
								<span className='text-xs lg:text-xs font-black text-primary'>
									{order.customer.charAt(0)}
								</span>
							</div>
							<div className='min-w-0 flex-1'>
								<p className='text-xs lg:text-xs font-semibold text-foreground truncate'>
									{order.customer}
								</p>
						...
						<div className='flex flex-col lg:flex-row items-end lg:items-center gap-2 lg:gap-3 flex-shrink-0'>
							<span className='text-xs lg:text-xs font-bold text-foreground whitespace-nowrap'>
								{order.amount}
							</span>
							<span
								className={cn(
