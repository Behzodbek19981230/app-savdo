...
export function renderReceiptHtml(props: ReceiptProps) {
    ...
    const rows = items
        .map((it, i) => {
            ...
            return `
        <tr>
          <td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${i + 1}</td>
          <td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${escapeHtml(model)}</td>
          <td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${escapeHtml(String(it.name || '-'))}</td>
          <td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${Number(it.quantity || 0)}</td>
					<td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${escapeHtml(unit)}</td>
          <td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${priceInUsd}</td>
          <td style="border:1px solid #000;padding:2px;text-align:center;font-size:9px">${totalPriceInUsd}</td>
        </tr>`;
        })
        .join('\n');
  ...
  <head>
    ...
    <style>
      ...
      body { font-family: "Times New Roman", serif; margin:0; padding:0; background:#fff }
      .page { width: 281mm; padding: 8mm; box-sizing: border-box }
    ...
    <div id="receipt-print" class="page">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
        ...
        <div style="flex:1;text-align:center;padding:0 8px">
          ...
          <div style="font-size:16px;font-weight:bold;color:red">${escapeHtml(customerName)}</div>
        </div>
        <div style="width:100px"></div>
      </div>
      <hr style="margin:10px 0 12px;border:0;border-top:1px solid #000" />

      <div style="display:flex;justify-content:space-between;gap:20px;font-size:11px;margin-bottom:8px">
        <div style="width:48%">
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold">Do'kon:</span><span style="text-align:right">Elegant</span></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold">Firma:</span><span style="text-align:right">Elegant</span></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold">Telefon nomer1:</span><span style="text-align:right">+99899-811-00-23</span></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold"></span><span style="text-align:right">+99890-812-94-44</span></div>
        </div>
        <div style="width:48%">
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold">Supermarket 1-2 Do'kon</span><span></span></div>
          ...
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold">Manzil:</span><span style="text-align:right">Toshkent viloyati, Chirchiq shahri</span></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:bold">Telefon:</span><span style="text-align:right">+99899-793-62-87</span></div>
        </div>
      </div>
      ...
      <div style="margin-top:12px;display:flex;justify-content:space-between;font-size:12px">
        <div style="width:60%">
          <div style="color:orange;font-weight:bold;font-size:13px">Ostatka ($): ${totalInUsd.toFixed(2)} $</div>
          <div style="font-size:11px">Olingan tovarlar summasi ($): ${totalInUsd.toFixed(2)} $</div>
          <div style="font-size:11px">Jami to'langan summa ($): ${paidInUsd.toFixed(2)} $</div>
        </div>
        <div style="width:38%;text-align:right;font-size:11px">To'langan summa dollarda ($): ${paidInUsd.toFixed(2)} $</div>
      </div>

      <div style="color:red;font-weight:bold;font-size:14px;margin-top:12px">Qolgan qarz ($): ${remainingInUsd.toFixed(2)} $</div>
    </div>
  </body>
  </html>`;
}
    ...
    return (
      ...
      `}</style>
            <div className='page'>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, fontSize: 11, marginBottom: 8 }}>
                    <div style={{ width: '48%' }}>
