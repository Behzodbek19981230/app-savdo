...
import { modelTypeService, type ModelType, type ModelTypeQueryParams } from '@/services/modelType.service';
...
export const useModelTypes = (params?: ModelTypeQueryParams) => {
	return useQuery({
		...
		enabled: params === undefined || params.madel !== undefined, // Only fetch when madel is provided or no params
	});
};
...
export const useCreateModelType = () => {
	...
	return useMutation({
		...
		onSuccess: async (createdType, variables) => {
			// Mos model bo'yicha listni yangilash
			const madel = variables.madel || createdType.madel;
			if (madel) {
				await queryClient.invalidateQueries({
					queryKey: modelTypeKeys.list({ madel, is_delete: false, limit: 1000 }),
				});
			}
			await queryClient.invalidateQueries({ queryKey: modelTypeKeys.lists() });
