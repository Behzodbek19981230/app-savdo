/**
 * Units Page (O'lchov birliklari)
 * O'lchov birliklari - unit API
 */
...
import { Search, ArrowUpDown, ArrowUp, ArrowDown, Plus, Pencil, Trash2, Loader2, Ruler, SearchIcon, X } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useUnits, useCreateUnit, useUpdateUnit, useDeleteUnit } from '@/hooks/api/useUnit';
...
type SortField = 'name' | 'code' | 'created_at' | null;
...
export default function Units() {
	...
	const handleOpenDialog = (item?: Unit) => {
		if (item) {
	...
	const onSubmit = async (data: UnitFormData) => {
		try {
			...
			if (editingId) {
				await updateUnit.mutateAsync({ id: editingId, data: submitData });
			} else {
	...
	useEffect(() => {
		if (createUnit.isSuccess || updateUnit.isSuccess) {
			setIsDialogOpen(false);
			setEditingId(null);
			form.reset();
		}
	}, [createUnit.isSuccess, updateUnit.isSuccess, form]);

	const handleDelete = async () => {
		if (!deletingId) return;
		try {
			await deleteUnit.mutateAsync(deletingId);
				...
				<CardHeader className='pb-4 flex flex-row items-center justify-between'>
					<div>
						<div className='flex items-center gap-2'>
							...
							<CardTitle className='text-lg'>O&apos;lchov birliklari</CardTitle>
						</div>
						<CardDescription>Jami {pagination?.total || 0} ta o&apos;lchov birligi</CardDescription>
					</div>
									...
									render={({ field }) => (
										<FormItem className='flex flex-row items-center justify-between rounded-lg border p-3'>
											<div className='space-y-0.5'>
												...
												<FormDescription>
													O&apos;lchov birligi faol yoki nofaol holatda
												</FormDescription>
											</div>
											<FormControl>
			...
			<AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Bu amalni qaytarib bo&apos;lmaydi. O&apos;lchov birligi butunlay o&apos;chiriladi.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
