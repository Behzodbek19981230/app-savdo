...
export default function DebtRepaymentsPage() {
    ...
    const { data: groupedData, isLoading, error } = useDebtRepaymentsGrouped(
        selectedFilialId
            ? {
    ...
    const handleModalSuccess = () => {
        // refresh listing
        // invalidate happens in hooks; simply re-run by changing appliedDateFrom or requery by refetch in future
    };
    ...
    const onSearchClients = async (q: string) => {
        ...
        setClientOptions(items.map((c: any) => ({ id: String(c.id), label: c.full_name || c.phone_number, value: String(c.id) })));
    };

    const onSearchEmployees = async (q: string) => {
        const res = await userService.getUsers({ search: q || '', limit: 100 } as any);
        const items = res.results || [];
        setEmployeeOptions(items.map((u: any) => ({ id: String(u.id), label: u.full_name || u.username, value: String(u.id) })));
    };
                                ...
                                <TableBody>
                                                ...
                                                {group.items.map((item: any, idx: number) => (
                                                    <TableRow key={item.id} className='hover:bg-muted/50'>
                                                        ...
                                                        <TableCell className='px-3 py-2'>
                                                            {item.created_time ? format(new Date(item.created_time), 'dd.MM.yyyy') : '-'}
                                                        </TableCell>
                                                        <TableCell className='px-3 py-2'>
                                                            {item.client_detail?.full_name || `ID: ${item.client}`}
                                                        </TableCell>
                                                        <TableCell className='px-3 py-2'>
                                                            {item.employee_detail?.full_name || (item.employee ? `ID: ${item.employee}` : '-')}
                                                        </TableCell>
                                                        ...
                                                        <TableCell className='px-3 py-2'>
                                                            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-semibold ${item.debt_status ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                                {item.debt_status ? "To'langan" : 'Kutilmoqda'}
                                                            </span>
                                                        </TableCell>
                                                        <TableCell className='px-3 py-2'>
                                                            <div className='flex items-center gap-2'>
                                                                <Button size='icon' variant='default' className='h-7 w-7 bg-amber-500 hover:bg-amber-600 text-white' onClick={() => openPdf(item.id)}>
                                                                    <Printer size={14} />
                                                                </Button>
                                    ...
                                    {groups.length > 0 && overallTotals.totalCount > 0 && (
                                        <TableRow className='bg-blue-50 font-semibold'>
                                            ...
                                            <TableCell />
                                            <TableCell />
                                            <TableCell className='text-right'>{formatCurrency(overallTotals.totalOldDebt)}</TableCell>
                                            <TableCell className='text-right'>{formatCurrency(overallTotals.totalNewDebt)}</TableCell>
                                            <TableCell className='text-right text-green-700'>{formatCurrency(overallTotals.totalPaid)}</TableCell>
                                            <TableCell colSpan={2} />
                                        </TableRow>
                                    )}
                                </TableBody>
