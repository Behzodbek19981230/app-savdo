...
export default function PurchaseInvoices() {
	...
	const skladsIncomingHasMore =
		!!skladsIncomingPagination && skladsIncomingPagination.lastPage > (skladsIncomingPagination.currentPage || 1);
							...
							onSubmit={newInvoiceForm.handleSubmit(async (values) => {
								...
								try {
									...
									const payload: CreatePurchaseInvoicePayload = {
										...
										date: values.date,
										sklad:
											invoiceType === PurchaseInvoiceType.EXTERNAL ? firstSkladId : values.sklad!,
										employee: user?.id || 0,
										filial: selectedFilialId || user?.filials_detail?.[0]?.id || 0,
										is_karzinka: true,
									};
