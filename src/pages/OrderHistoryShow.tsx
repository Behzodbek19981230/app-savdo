...
function GivenCountCell({
    ...
    return (
        ...
        >
            <span className={`text-base font-bold ${isDifferent ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-slate-100'}`}>
                {givenCount}
            </span>
            {isDifferent && (
                <AlertTriangle className='h-4.5 w-4.5 text-red-500 flex-shrink-0' title={`Soni: ${count}, Berilgan: ${givenCount}`} />
            )}
