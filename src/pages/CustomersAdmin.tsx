...
import { ArrowDown, ArrowUp, ArrowUpDown, Edit, Loader2, MapPin, Plus, Search, Trash2, Users } from 'lucide-react';
...
export default function CustomersAdmin() {
	...
	const getTypeLabel = (type?: string | null) => {
		if (type === 'dona') return 'Dona';
		if (type === 'optom') return 'Optom';
		if (type === 'dokon') return 'Dokon';
		if (type === 'hamkor') return 'Hamkor';
		return '-';
	};
									...
									<TableBody>
										{clients.map((client, index) => (
											<TableRow key={client.id}>
												...
												<TableCell>{client.date_of_birthday || '-'}</TableCell>
												...
												<TableCell>{client.region_detail?.name || '-'}</TableCell>
												<TableCell>{client.district_detail?.name || '-'}</TableCell>
