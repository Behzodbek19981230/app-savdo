			...
			<Card className='border-border/50 shadow-sm rounded-xl lg:rounded-2xl'>
				<CardHeader className='flex flex-col gap-4 p-4 lg:p-6'>
					<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
						...
						<div className='relative w-full sm:w-64 lg:w-80'>
							...
							<Input
								...
								className='pl-10 w-full text-xs h-7 rounded-lg'
							/>
						</div>
					</div>
					{/* Mobile Sort */}
					<div className='md:hidden'>
						<Autocomplete
							options={[
								{ value: 'none', label: "Saralash yo'q" },
								{ value: 'name', label: "Ism bo'yicha" },
								{ value: 'email', label: "Email bo'yicha" },
								{ value: 'orders', label: 'Buyurtmalar soni' },
								{ value: 'totalSpent', label: 'Jami xarajat' },
								{ value: 'status', label: 'Holat' },
								{ value: 'joinDate', label: "Qo'shilgan sana" },
							]}
					...
					{sortedCustomers.length > 0 && (
						<div className='p-4 lg:p-5 border-t border-border/50'>
							<div className='flex flex-col sm:flex-row items-center justify-between gap-4'>
								<p className='text-xs lg:text-xs text-muted-foreground order-2 sm:order-1'>
									{startIndex + 1}-{Math.min(endIndex, sortedCustomers.length)} dan{' '}
									{sortedCustomers.length} ta ko'rsatilmoqda
								</p>
								<div className='order-1 sm:order-2'>{renderPagination()}</div>
							</div>
						</div>
					)}
				</CardContent>
