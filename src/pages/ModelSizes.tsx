...
export default function ModelSizes() {
	...
	const isMutating = createModelSize.isPending || updateModelSize.isPending || deleteModelSize.isPending;
			...
			<Card>
				<CardHeader className='pb-4 flex flex-row items-center justify-between'>
					<div>
						...
						<CardDescription>Jami {pagination?.total || 0} ta model o'lchami</CardDescription>
					</div>
											...
											<TableRow key={modelSize.id}>
												...
												<TableCell>
													<Badge variant='secondary'>
														{modelSize.size}{' '}
														{modelSize.unit
															? `(${units.find((u) => u.id === modelSize.unit)?.code || ''})`
															: ''}
													</Badge>
												</TableCell>
