...
import { useDistricts, useRegions, useDistrict } from '@/hooks/api/useLocations';
...
export default function Users() {
    ...
    const { data: districtsData } = useDistricts(
        selectedRegion && selectedRegion > 0 ? { region: selectedRegion, perPage: 1000 } : undefined,
    );
    // Fetch selected district separately if it's not in the filtered list
    const { data: selectedDistrictData } = useDistrict(
        selectedDistrict && selectedDistrict > 0 && (!districtsData?.results?.some((d) => d.id === selectedDistrict))
            ? selectedDistrict
            : 0,
    );
    ...
    // Combine districts from filtered list and selected district if not in list
    const districts = useMemo(() => {
        const districtsList = districtsData?.results || [];
        const selectedDistrictItem = selectedDistrictData ? [selectedDistrictData] : [];
        if (selectedDistrictItem.length > 0 && !districtsList.some((d) => d.id === selectedDistrictItem[0].id)) {
            return [...districtsList, ...selectedDistrictItem];
        }
        return districtsList;
    }, [districtsData?.results, selectedDistrictData]);
    const companies = companiesData?.results || [];

    const getCompaniesNames = (companyIds: number[] | null | undefined) => {
        if (!companyIds || companyIds.length === 0) return '-';
        const names = companyIds.map((id) => companies.find((c) => c.id === id)?.name || String(id)).filter(Boolean);
    ...
    const formatRoles = (u: AppUser) => {
        if (Array.isArray(u.roles) && u.roles.length > 0) {
            const names = u.roles.map((id) => roles.find((r) => r.id === id)?.name || String(id)).filter(Boolean);
    ...
    // Reset district when region changes (but not when editing and region is already set)
    const prevRegionRef = useRef<number | undefined>(undefined);
    useEffect(() => {
        // Only reset if region actually changed (not on initial load)
        if (prevRegionRef.current !== undefined && selectedRegion && prevRegionRef.current !== selectedRegion && editingId === null) {
    ...
    useEffect(() => {
        if (editingId && userDetailData && isDialogOpen) {
            ...
            form.reset({
                ...userData,
                region: userDetailData.region || 0,
                district: userDetailData.district || 0,
    ...
    const handleOpenDialog = (item?: AppUser) => {
        if (item) {
            ...
            form.setValue('region', item.region || 0);
            form.setValue('district', item.district || 0);
    ...
    const onSubmit = async (values: UserFormData) => {
        ...
        const payload: CreateAppUserPayload = {
            ...
            order_filial: values.order_filial,
        };
                            ...
                            <div className='grid gap-4 py-4'>
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent
                                                        align='start'
                                                        className='w-[320px] max-h-[320px] overflow-y-auto'
                                                    >
                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                            ...
                                            <FormItem>
                                                ...
                                                <DropdownMenu>
                                                        ...
                                                        <FormControl>
                                                            ...
                                                            >
                                                                {field.value && field.value.length > 0
                                                                    ? `${field.value.length} ta tanlandi`
                                                                    : 'Tanlang'}
                                                    ...
                                                    <DropdownMenuContent className='w-56 max-h-60 overflow-y-auto'>
                                                        <DropdownMenuLabel>Filiallar</DropdownMenuLabel>
                                                        <DropdownMenuSeparator />
                                                        {companies.map((c) => (
                                                            <DropdownMenuCheckboxItem
                                                                key={c.id}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>Savdo filiali</FormLabel>
                                                <Select
                                                    onValueChange={(v) => {
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <Select
                                                    onValueChange={(v) => field.onChange(Number(v))}
                                                    value={String(field.value ?? 0)}
                                                    disabled={!selectedRegion || selectedRegion === 0}
                                                >
                                                    <FormControl>
                                                        <SelectTrigger>
                                                            <SelectValue placeholder={!selectedRegion || selectedRegion === 0 ? 'Avval viloyatni tanlang' : 'Tanlang'} />
                                                        </SelectTrigger>
                                                    </FormControl>
                                                    <SelectContent>
                                                        <SelectItem value='0'>-</SelectItem>
                                                ...
                                                <div className='flex gap-2'>
                                                    ...
                                                    {avatarFile && (
                                                        <Button
                                                            type='button'
                                                            variant='ghost'
                                                            onClick={() => setAvatarFromFile(null, null)}
                                                        >
                                                            Tozalash
                                                        </Button>
                                                    )}
