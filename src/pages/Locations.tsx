...
export default function Locations() {
	...
	const { data: districtsData, isLoading: districtsLoading } = useDistricts({
		...
		region:
			activeTab === 'district' && selectedRegionFilter && selectedRegionFilter !== 'all'
				? Number(selectedRegionFilter)
				: undefined,
	});
	...
	const handleOpenDialog = (item?: Region | District) => {
		if (item) {
			const regionValue: number | undefined =
				'region' in item ? (typeof item.region === 'number' ? item.region : item.region_detail?.id) : undefined;
							...
							<div className='mb-4 flex gap-3'>
								...
								{activeTab === 'district' && (
									<Autocomplete
										...
										onValueChange={(v) => setSelectedRegionFilter(String(v))}
				...
				<DialogContent className='sm:max-w-[525px]'>
					<form onSubmit={handleSubmit}>
						...
						<div className='grid gap-4 py-4'>
							...
							{activeTab === 'district' && (
								<div className='grid gap-2'>
									...
									<Autocomplete
										options={allRegions.map((region) => ({ value: region.id, label: region.name }))}
										value={editingItem?.region || undefined}
										onValueChange={(v) =>
											setEditingItem((prev) => ({ ...prev!, region: Number(v) }))
										}
						...
						<DialogFooter>
							...
							<Button type='submit' disabled={isMutating} className=''>
