...
import {
	modelTypeService,
	type ProductTypeCreateItem,
	type ProductTypeSizeItem,
...
export default function ModelTypeAndSize({ defaultTab = 'model-type' }: ModelTypeAndSizeProps) {
	...
	const [productTypeSizeRows, setProductTypeSizeRows] = useState<ProductTypeSizeItem[]>([
		defaultSizeRow(),
	]);
	...
	const modelsEdit = modelsEditData?.results ?? [];
	...
	const removeSizeRow = (index: number) =>
		setProductTypeSizeRows((prev) => prev.filter((_, i) => i !== index));
	const updateSizeRow = (index: number, field: 'size' | 'unit', value: string | number) =>
		setProductTypeSizeRows((prev) =>
	...
	const handleSubmitEdit = (e: React.FormEvent) => {
		...
		updateModelType.mutate(
			{
				id: editingId,
				data: { name: editName.trim(), madel: editMadel || undefined, sorting: editSorting },
			},
			...
			<Dialog open={isEditModalOpen} onOpenChange={(open) => !open && closeEditModal()}>
				<DialogContent className="sm:max-w-[425px]">
					<DialogHeader>
						...
						<DialogDescription>Nomi, model va tartibni o'zgartiring.</DialogDescription>
					</DialogHeader>
