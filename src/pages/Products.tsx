...
import { useCreateProductCategory, useProductCategories } from '@/hooks/api/useProductCategories';
import { useProductBranchCategories } from '@/hooks/api/useProductBranchCategories';
import { useCreateProductModel, useProductModels } from '@/hooks/api/useProductModels';
import { useCreateModelType, useModelTypes } from '@/hooks/api/useModelTypes';
...
export default function Products() {
	...
	const [images, setImages] = useState<ProductAttachment[]>([]);

	// Filters
	const [filterBranch, setFilterBranch] = useState<string>('');
	const [filterBranchCategory, setFilterBranchCategory] = useState<string>('');
	...
	const { data, isLoading } = useProducts({
		...
		branch: filterBranch && filterBranch !== 'all' ? parseInt(filterBranch) : undefined,
		branch_category:
	...
	const modelsInfinite = useInfiniteQuery({
		queryKey: [
			'productModels',
			'list',
			{ branch_category: selectedBranchCategory, is_delete: false, search: modelSearch },
		],
		queryFn: ({ pageParam }) =>
	...
	// Filter infinite queries
	const filterBranchesInfinite = useInfiniteQuery({
		queryKey: ['productCategories', 'filter', { is_delete: false, search: filterBranchSearch }],
		queryFn: ({ pageParam }) =>
			productCategoryService.getCategories({
				page: pageParam,
				limit: AUTOCOMPLETE_PAGE_SIZE,
				search: filterBranchSearch || undefined,
				is_delete: false,
			}),
		getNextPageParam: (lastPage) =>
			lastPage.pagination.currentPage < lastPage.pagination.lastPage
	...
	const filterBranchCategoriesInfinite = useInfiniteQuery({
		queryKey: [
			'productBranchCategories',
			'filter',
			{
				product_branch: filterBranch && filterBranch !== 'all' ? parseInt(filterBranch) : undefined,
				is_delete: false,
				search: filterBranchCategorySearch,
			},
		],
		queryFn: ({ pageParam }) =>
			productBranchCategoryService.getCategories({
	...
	const filterModelTypesInfinite = useInfiniteQuery({
		queryKey: [
			'modelTypes',
			'filter',
			{
				madel: filterModel && filterModel !== 'all' ? parseInt(filterModel) : undefined,
				is_delete: false,
				search: filterModelTypeSearch,
			},
		],
		queryFn: ({ pageParam }) =>
			modelTypeService.getModelTypes({
				page: pageParam,
				limit: AUTOCOMPLETE_PAGE_SIZE,
				search: filterModelTypeSearch || undefined,
				is_delete: false,
				madel: filterModel && filterModel !== 'all' ? parseInt(filterModel) : undefined,
			}),
		getNextPageParam: (lastPage) =>
	...
	// Filter options
	const filterBranches = filterBranchesInfinite.data?.pages.flatMap((p) => p.results) ?? [];
	const filterBranchCategoriesList = filterBranchCategoriesInfinite.data?.pages.flatMap((p) => p.results) ?? [];
	const filterModelsList = filterModelsInfinite.data?.pages.flatMap((p) => p.results) ?? [];
	const filterModelTypesList = filterModelTypesInfinite.data?.pages.flatMap((p) => p.results) ?? [];

	const filterBranchOptions = filterBranches.map((b) => ({ value: b.id, label: b.name }));
	const filterBranchCategoryOptions = filterBranchCategoriesList.map((c) => ({
		value: c.id,
		label: c.name,
	}));
	const filterModelOptions = filterModelsList.map((m) => ({ value: m.id, label: m.name }));
	const filterModelTypeOptions = filterModelTypesList.map((t) => ({ value: t.id, label: t.name }));
	...
	const handleOpenImagesDialog = async (item: Product) => {
		setImagesProduct(item);
		const data = await productService.getProductImages(item.id);
		setImages(data);
	...
	// Total stock count for the stock dialog summary
	const totalStockCount = stockItems.reduce((acc, s) => acc + (Number(s?.count) || 0), 0);
	...
	return (
		<div className='space-y-6'>
				...
				<CardHeader>
					<div className='flex items-center justify-between'>
						<div className='flex items-center gap-3'>
							<Package className='h-7 w-6 text-primary' />
							<div>
				...
				<CardContent>
					...
					<div className='mb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3'>
						...
						<Autocomplete
							...
							value={filterBranch && filterBranch !== 'all' ? parseInt(filterBranch) : undefined}
							onValueChange={(val) => {
								if (val) {
									setFilterBranch(String(val));
								} else {
									setFilterBranch('');
								}
								setFilterBranchCategory('');
								setFilterModel('');
								setFilterModelType('');
								setCurrentPage(1);
							}}
							...
							onSearchChange={setFilterBranchSearch}
							onScrollToBottom={() => filterBranchesInfinite.fetchNextPage()}
							hasMore={!!filterBranchesInfinite.hasNextPage}
							isLoadingMore={filterBranchesInfinite.isFetchingNextPage}
							isLoading={filterBranchesInfinite.isLoading}
						/>

						<Autocomplete
							options={filterBranchCategoryOptions}
							value={
								filterBranchCategory && filterBranchCategory !== 'all'
									? parseInt(filterBranchCategory)
									: undefined
							}
							onValueChange={(val) => {
								if (val) {
									setFilterBranchCategory(String(val));
								} else {
									setFilterBranchCategory('');
								}
								setFilterModel('');
							...
							onScrollToBottom={() => filterBranchCategoriesInfinite.fetchNextPage()}
							hasMore={!!filterBranchCategoriesInfinite.hasNextPage}
							isLoadingMore={filterBranchCategoriesInfinite.isFetchingNextPage}
							isLoading={filterBranchCategoriesInfinite.isLoading}
						/>

						<Autocomplete
							options={filterModelOptions}
							value={filterModel && filterModel !== 'all' ? parseInt(filterModel) : undefined}
							onValueChange={(val) => {
								if (val) {
									setFilterModel(String(val));
								} else {
									setFilterModel('');
								}
								setFilterModelType('');
						...
						<Autocomplete
							...
							value={filterModelType && filterModelType !== 'all' ? parseInt(filterModelType) : undefined}
							onValueChange={(val) => {
								if (val) {
									setFilterModelType(String(val));
								} else {
									setFilterModelType('');
								}
								setCurrentPage(1);
							}}
							placeholder='Model turi'
							searchPlaceholder='Model turi qidirish...'
							emptyText='Model turi topilmadi'
							disabled={!filterModel || filterModel === 'all'}
							onSearchChange={setFilterModelTypeSearch}
							onScrollToBottom={() => filterModelTypesInfinite.fetchNextPage()}
							hasMore={!!filterModelTypesInfinite.hasNextPage}
						...
						<div>
							<Autocomplete
								...
								onValueChange={(val) => {
									if (val && val !== 'all') setFilterStatus(String(val));
					...
					{(filterBranch ||
						...
						<div className='mb-4 flex flex-wrap items-center gap-2 p-3 bg-muted/50 rounded-lg border'>
							<span className='text-xs font-medium text-muted-foreground'>Faol filtrlar:</span>
							{filterBranch && filterBranch !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									<span className='text-xs'>Bo&apos;lim:</span>
									<span className='font-medium'>
										{filterBranches.find((b) => b.id.toString() === filterBranch)?.name}
									</span>
									<button
										onClick={() => {
											setFilterBranch('');
							...
							{filterBranchCategory && filterBranchCategory !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									<span className='text-xs'>Kategoriya turi:</span>
									<span className='font-medium'>
										{
											filterBranchCategoriesList.find(
												(c) => c.id.toString() === filterBranchCategory,
											)?.name
										}
									</span>
									<button
							...
							{filterModel && filterModel !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									...
									<span className='font-medium'>
										{filterModelsList.find((m) => m.id.toString() === filterModel)?.name}
									</span>
									<button
										onClick={() => {
											setFilterModel('');
							...
							{filterModelType && filterModelType !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									...
									<span className='font-medium'>
										{filterModelTypesList.find((mt) => mt.id.toString() === filterModelType)?.name}
									</span>
									<button
										onClick={() => {
											setFilterModelType('');
												...
												<TableCell className='text-right'>
													<Badge
														...
														onClick={async () => {
															...
															try {
																const res: any = await api.get('/product-stock', {
																	params: { product: product.id },
																});
																setStockItems(res?.results ?? res ?? []);
															} catch (err: any) {
																toast({
																	title: 'Xatolik',
																	description: err?.message || 'Stockni olishda xato',
																});
															} finally {
																setStockLoading(false);
															}
														}}
														title='Omborlardagi miqdor'
													>
														{product.count}
													</Badge>
												</TableCell>
			...
			{/* Stock Dialog */}
			<Dialog open={isStockDialogOpen} onOpenChange={(v) => setIsStockDialogOpen(v)}>
				<DialogContent className='w-[90vw] max-w-lg'>
					<DialogHeader>
						<div className='flex items-center gap-3'>
							...
							<div className='h-12 w-12 rounded overflow-hidden bg-muted flex items-center justify-center'>
								{stockProduct?.images?.file ? (
									<img
										src={stockProduct.images.file}
										alt={stockProduct.model_detail?.name || 'product'}
										className='h-full w-full object-cover'
									/>
								) : (
									<Package className='h-7 w-6 text-muted-foreground' />
								)}
							</div>
							<div>
								<DialogTitle>Omborlardagi miqdor</DialogTitle>

								<div className='text-xs text-muted-foreground mt-1'>
									Jami mavjud: <span className='font-semibold'>{totalStockCount}</span>
								</div>
							</div>
						</div>
					</DialogHeader>
					...
					<div className='py-3'>
						{stockLoading ? (
							...
							<div className='flex flex-col items-center justify-center py-8 text-center'>
								<Package className='h-7 w-10 text-muted-foreground/60 mb-3' />
								<p className='text-xs text-muted-foreground'>Ombor ma'lumotlari topilmadi</p>
							</div>
						) : (
								...
								{stockItems.map((s) => (
									...
									>
										<div className='flex items-start gap-3'>
											<div className='h-7 w-9 rounded bg-muted/60 flex items-center justify-center text-xs font-medium'>
												{s.sklad_detail?.name?.charAt(0) ?? 'S'}
											</div>
											<div>
												<div className='font-medium'>
													{s.sklad_detail?.name || `Sklad ${s.sklad}`}
												</div>
												{(s.sklad_detail?.address || s.sklad_detail?.phone) && (
										...
										<div className='flex items-center gap-3'>
											<div className='text-xs text-muted-foreground'>Miqdori</div>
				...
				<DialogContent className='w-[95vw] max-w-[800px] max-h-[90vh] overflow-y-auto'>
					...
					{editingId && isProductByIdLoading ? (
						<div className='flex items-center justify-center py-12'>
							<Loader2 className='h-7 w-10 animate-spin text-muted-foreground' />
						</div>
					) : (
						<Form {...form}>
							<form
													...
													<FormControl>
														<div className='flex w-[200px]'>
															<Autocomplete
																...
																onScrollToBottom={() =>
																	productTypeSizesInfinite.fetchNextPage()
																}
																hasMore={!!productTypeSizesInfinite.hasNextPage}
																isLoadingMore={
																	productTypeSizesInfinite.isFetchingNextPage
																}
															/>
														</div>
													</FormControl>
							...
							>
								{({ zoomIn, zoomOut, resetTransform }) => (
									<div className='relative'>
										...
										{/* Image Counter */}
										{images.length > 1 && (
											<div className='absolute bottom-2 right-2 z-10 bg-background/80 px-2 py-1 rounded text-xs'>
