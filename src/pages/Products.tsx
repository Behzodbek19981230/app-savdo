...
export default function Products() {
    ...
    // Watch selected model_type for filtering modelSizes
    const selectedModelType = form.watch('model_type');

    const { data: modelSizesData } = useModelSizes({
        perPage: 1000,
        is_delete: false,
        model_type: selectedModelType || undefined,
    });
    ...
    const handleOpenDialog = (item?: Product) => {
        if (item) {
            ...
            form.reset({
                category: item.branch || 0,
                model: item.model || 0,
                model_type: item.type || 0,
                model_size: item.size || 0,
                size: item.size || 0,
                type: ProductType.DONA,
                count: item.count || 0,
                real_price: typeof item.real_price === 'string' ? parseFloat(item.real_price) : item.real_price || 0,
                price: typeof item.unit_price === 'string' ? parseFloat(item.unit_price) : item.unit_price || 0,
                sorting: null,
    ...
    }, [selectedModelType, isDialogOpen, form]);
                                    ...
                                    <TableBody>
                                        {products.map((product, index) => (
                                            <TableRow key={product.id}>
                                                ...
                                                <TableCell>{product.size_detail?.size ?? '-'} {product.size_detail?.unit_detail?.name ?? ''}</TableCell>
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                >
                                                    <FormControl>
                                                        <SelectTrigger>
                                                            <SelectValue placeholder={selectedModelType ? 'Tanlang' : 'Avval model turini tanlang'} />
                                                        </SelectTrigger>
                                                    </FormControl>
                                                    <SelectContent>
                                                        {modelSizes.length === 0 ? (
                                                            ...
                                                            modelSizes.map((modelSize) => (
                                                                ...
                                                                >
                                                                    {modelSize.size} {modelSize.unit_detail?.code || ''}
                                                                </SelectItem>
                                                            ))
                                                        )}
                                                    </SelectContent>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription>
                                                Mahsulot haqida batafsil ma'lumot (ixtiyoriy)
                                            </FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
