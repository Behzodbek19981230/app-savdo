			...
			<Card>
				...
				<CardContent>
					...
					<div className='mb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3'>
						...
						<Select
							value={filterBranchCategory}
							onValueChange={(value) => {
								setFilterBranchCategory(value);
								setFilterModel('');
								setFilterModelType('');
								setCurrentPage(1);
							}}
						...
						>
							...
							<SelectContent>
								...
								{filterModelTypes.map((modelType) => (
						...
						<div className='mb-4 flex flex-wrap items-center gap-2 p-3 bg-muted/50 rounded-lg border'>
							...
							{filterBranch && filterBranch !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									...
									<span className='font-medium'>
										{branches.find((b) => b.id.toString() === filterBranch)?.name}
									</span>
									<button
										onClick={() => {
											setFilterBranch('');
							...
							{filterBranchCategory && filterBranchCategory !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									...
									<span className='font-medium'>
										{
											filterBranchCategories.find((c) => c.id.toString() === filterBranchCategory)
												?.name
										}
									</span>
									<button
							...
							{filterModel && filterModel !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									...
									<span className='font-medium'>
										{filterModels.find((m) => m.id.toString() === filterModel)?.name}
									</span>
									<button
										onClick={() => {
											setFilterModel('');
							...
							{filterModelType && filterModelType !== 'all' && (
								<Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
									...
									<span className='font-medium'>
										{filterModelTypes.find((mt) => mt.id.toString() === filterModelType)?.name}
									</span>
									<button
										onClick={() => {
											setFilterModelType('');
