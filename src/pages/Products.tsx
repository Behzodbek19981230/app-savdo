...
export default function Products() {
    ...
    const filterBranchCategoriesInfinite = useInfiniteQuery({
        ...
        queryFn: ({ pageParam }) =>
            productBranchCategoryService.getCategories({
                ...
                is_delete: false,
                product_branch:
                    filterBranch && filterBranch !== 'all' ? parseInt(filterBranch) : undefined,
            } as Parameters<typeof productBranchCategoryService.getCategories>[0]),
        getNextPageParam: (lastPage) =>
            lastPage.pagination.currentPage < lastPage.pagination.lastPage
                ? lastPage.pagination.currentPage + 1
    ...
    const filterBranchCategoriesList =
        filterBranchCategoriesInfinite.data?.pages.flatMap((p) => p.results) ?? [];
                        ...
                        <div className='mb-4 flex flex-wrap items-center gap-2 p-3 bg-muted/50 rounded-lg border'>
                            ...
                            {filterModelType && filterModelType !== 'all' && (
                                <Badge variant='secondary' className='gap-1.5 pl-2 pr-1'>
                                    ...
                                    <span className='font-medium'>
                                        {
                                            filterModelTypesList.find((mt) => mt.id.toString() === filterModelType)
                                                ?.name
                                        }
                                    </span>
                                    <button
