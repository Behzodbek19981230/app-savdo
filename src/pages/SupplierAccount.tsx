...
import { Loader2, Package, SearchIcon, X, Plus, Pencil, Trash2 } from 'lucide-react';
							...
							<TableBody>
								{items.map((it: any, idx: number) => (
									<TableRow key={it.id}>
										...
										<TableCell>{it.supplier_detail?.name || `#${it.supplier || '-'}`}</TableCell>
										...
										<TableCell>
											<div className='flex items-center '>
												<Button
													...
													size='sm'
													onClick={() => onEdit(it)}
													className=' p-0 text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50 dark:hover:bg-yellow-950/30'
												>
					...
					<Table>
						...
						<TableBody>
							{items.map((it: any, idx: number) => (
								<TableRow key={it.id}>
									<TableCell className='font-medium'>{items.length - idx}</TableCell>
									<TableCell>{it.date ? moment(it.date).format('YYYY-MM-DD') : '-'}</TableCell>
									<TableCell>
										{it.employee_detail?.full_name ||
											it.employee_detail?.username ||
											`#${it.employee || '-'}`}
									</TableCell>
									<TableCell>{it.supplier_detail?.name || `#${it.supplier || '-'}`}</TableCell>
									<TableCell className='text-right'>{formatCurrency(it.total_debt_old)}</TableCell>
									<TableCell className='text-right'>{formatCurrency(it.total_debt)}</TableCell>
									<TableCell className='text-right text-blue-600 dark:text-blue-400 font-semibold'>
									...
									<TableCell className='text-right'>{formatCurrency(it.summa_terminal)}</TableCell>
									<TableCell className='text-right'>{formatCurrency(it.summa_transfer)}</TableCell>
									<TableCell>
										<div className='flex items-center '>
											<Button
												variant='ghost'
								...
								<>
									...
									{group.items.map((it: any, idx: number) => (
										<TableRow key={it.id}>
											...
											{activeTab === 'account' && (
												<TableCell>
													{it.purchase_invoice_detail?.invoice_number ||
														`#${it.purchase_invoice || '-'}`}
												</TableCell>
											)}
											<TableCell className='text-right text-blue-600 dark:text-blue-400 font-semibold'>
...
export default function SupplierAccountPage() {
	...
	useEffect(() => {
		...
		// Bo'sh qidiruvda listni saqlash clear qilinganda bo'sh qolishni oldini oladi.
		if (formSupplierSearch.trim()) setSupplierOptions([]);
	...
	useEffect(() => {
		if (accountData && editingAccountId) {
			accountForm.reset({
				...
				total_turnover:
					typeof accountData.total_turnover === 'string'
						? parseFloat(accountData.total_turnover)
						: (accountData.total_turnover as number) || 0,
				filial_debt:
					typeof accountData.filial_debt === 'string'
						? parseFloat(accountData.filial_debt)
						: (accountData.filial_debt as number) || 0,
			});
		}
	...
	const isAccountDirectArray =
		accountResults.length > 0 && !(accountResults[0] as any)?.date && !(accountResults[0] as any)?.items;
						...
						<form onSubmit={accountForm.handleSubmit(handleSubmitAccount)} className='space-y-4'>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>
											Jami aylanma <span className='text-destructive'>*</span>
										</FormLabel>
										<FormControl>
											<NumberInput
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel>
											Filial qarzi <span className='text-destructive'>*</span>
										</FormLabel>
										<FormControl>
											<NumberInput
												value={String(field.value ?? 0)}
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Eski qarz <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Jami qarz <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Jami to'lov ($) <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Dollar to'lovi <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Naqt to'lovi <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Kilik to'lovi <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Terminal to'lovi <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>
												Transfer to'lovi <span className='text-destructive'>*</span>
											</FormLabel>
											<FormControl>
												<NumberInput
													value={String(field.value ?? 0)}
